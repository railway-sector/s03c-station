...
return -0.5 * dot(conic, vec3(x * x, 2.0 * x * y, y * y));
  ...
  `)}const Zt=""+(se-1),Qt=`${Math.log2(se)}`,La=""+(ye-1),Ha=`${Math.log2(ye)}`;let mi=class extends Tt{constructor(){super(...arguments),this.tileCameraPosition=B(),this.cameraDelta=B()}};function gi(n){n.code.add(x`float unpackOpacity(uvec4 packedGaussian) {
...
vec3 additionalLight = mainLightIntensity * additionalAmbientScale * ambientBoostFactor * lightingGlobalFactor;`,l=x`
  ...
  `),s}function Jt(n){const{clippingEnabled:e,hasSlicePlane:t,receiveShadows:i}=n;return i||t||e}const Wa=Dt(),Na=Object.freeze(Object.defineProperty({__proto__:null,GaussianSplatPassParameters:Ft,build:xi},Symbol.toStringTag,{value:"Module"}));let Gt=class extends Tt{};function yi(n){const e=new tt;e.include(ri);const{hasEmission:t}=n,i=e.fragment;return t&&i.include(Ji,n),i.uniforms.add(new Le("colorTexture",a=>a.color),new Le("splatOutputColor",a=>a.splatColor)),t&&i.uniforms.add(new Le("emissionTexture",a=>a.emission)),e.outputs.add("fragColor","vec4",0),t&&e.outputs.add("fragEmission","vec4",1),e.fragment.main.add(x`
